...
function RobotDynamics.dynamics(model::Human_QuadRotors, x, u)
    ...
    humans_pose_dot = [u[5]*cos(x[17]); u[5]*sin(x[17]); 0; u[6]]

    [r1dot; q1dot; v1dot; w1dot; humans_pose_dot]
end

RobotDynamics.state_dim(::Human_QuadRotors) = 17
RobotDynamics.control_dim(::Human_QuadRotors) = 6
...
u_quad = 3
add_constraint!(cons, BoundConstraint(n,m, u_min=[0,0,0,0,-1,-1],
                    u_max=[u_quad,u_quad,u_quad,u_quad, 1,1]), 1:N-1)
