...
def perform_experiment():
    ...
    cf2.goTo(cf2_start_pos, yaw=0.0, duration=5.0)
    ...
    for i in range(LOOP_ITERS):
        ...
        if USE_JULIA:
            ...
            cf2.goTo(xd, yaw=0.0, duration=GOTO_DURATION)

            try:
                (pos_cf1,rot_cf1) = listener.lookupTransform('/world', '/cf2', rospy.Time(0))
            except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
                pass

            try:
                (pos_human1,rot_human1) = listener.lookupTransform('/world', '/cf1', rospy.Time(0))
            except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
                pass

            try:
                (pos_human2,rot_human2) = listener.lookupTransform('/world', '/cf3', rospy.Time(0))
            except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
                pass
            ...
            yaw_human1 = np.arctan2(2.0 * (w1 * z1 + x1 * y1), 1.0 - 2.0 * (y1 * y1 + z1 * z1));
            ...
            yaw_human2 = np.arctan2(2.0 * (w2 * z2 + x2 * y2), 1.0 - 2.0 * (y2 * y2 + z2 * z2));
            ...
            # x_update = [pos_cf1[0], pos_cf1[1], pos_cf1[2], rot_cf1[3], rot_cf1[0], rot_cf1[1], rot_cf1[2],
            #            xd_actual[13],xd_actual[14],xd_actual[15],xd_actual[16],
            #            xd_actual[17],xd_actual[18],xd_actual[19],xd_actual[20]]

            x_update = [pos_cf1[0], pos_cf1[1], pos_cf1[2], rot_cf1[3], rot_cf1[0], rot_cf1[1], rot_cf1[2],
                        pos_human1[0],pos_human1[1],pos_human1[2],yaw_human1,
                        pos_human2[0],pos_human2[1],pos_human2[2],yaw_human2]
...
if __name__ == '__main__':
    ...
    if USE_JULIA:
        ...
        Main.include("Quadrotor_two_humans.jl")
